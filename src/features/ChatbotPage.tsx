...
const ChatbotPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const savedId = loadConsultationId();
    setMessages(saved);
  ...
  const handleSaveSummaryToDB = async (consultationId: number, summary: string, issue: string, templateId: number) => {
    try {
  ...
  const handleSend = async (input: string) => {
    ...
    try {
        ...
        if (answer.includes('문서 작성 모드로 전환합니다')) {
          ...
          if (jsonMatch && consultationId) {
            try {
              ...
              const templateId = 1; // TODO: 실제 템플릿 ID 매핑
              await handleSaveSummaryToDB(consultationId, summary, issue, templateId);
            } catch (e) {
      ...
      setMessages((prev) => [
        ...prev.filter((msg) => msg.text !== '__typing__'),
  ...
  const lastBotMessage = messages
    .filter((msg) => msg.sender === 'bot' && msg.text && msg.text !== '__typing__')
    .slice(-1)[0];
  ...
  return (
    <div className="flex flex-col h-[calc(85vh-4rem)]">
      ...
      >
        ...
        {messages
          .filter((msg) => msg.text.trim() !== '' || msg.text === '__typing__')
          .map((msg, idx) => (
            <ChatBubble
              key={idx}
              sender={msg.sender}
        ...
        {autoGenerateData && (
          <div className="text-center mt-6">
            <button
              ...
              onClick={() => {
                navigate('/edit', { state: autoGenerateData });
              }}
            >
              ✍️ 자동 문서 작성하기
            </button>
